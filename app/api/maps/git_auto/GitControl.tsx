...
const GIT_CACHE_KEYS = {
  ...
  diff: "diff",
  history: "history",
  dashboardState: "git_auto:dashboard-state",
...
function Workflow({
  ...
  const currentPullRequest = workflow?.pull_request;
  const prState = currentPullRequest?.state?.toLowerCase();
  const steps = ["Fetch", "Pull", "Commit", "Push", "Pull Request"];
  const states = [
    ...(newCycle
      ? ["Waiting", "Waiting", "Waiting", "Waiting", pullRequestLoading ? "Running" : "Waiting"]
      : [
    fetching
      ? "Running"
      : !workflow
...
function GitAutoConfigDemo({
  ...
  const [config, setConfig] = useState<GitAutoConfig>({
    provider: "",
    remote_url: "",
    repository: "",
    component: "all",
  ...
  const reloadConfig = useCallback(async () => {
    ...
    try {
      ...
      laravelGitService.setConfig(current);
      setConfig((old) => ({ ...old, ...current }));
      onUsernameChange(current.username || "");
      setMessage("Đã tải lại cấu hình");
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Không lấy được cấu hình hiện tại");
    } finally {
      setReloading(false);
    }
  }, [onUsernameChange]);
  ...
  const update = (key: keyof GitAutoConfig, value: string) =>
  ...
  const save = async () => {
    ...
    try {
      ...
      onNotify("Đã lưu và kết nối cấu hình thành công");
      onSaved();
    } catch (error) {
      const message = error instanceof Error ? error.message : "Lưu cấu hình thất bại";
      setMessage(message);
      onNotify(`Lưu cấu hình thất bại: ${message}`);
    } finally {
      setSaving(false);
    }
  };
        ...
        <div className="min-w-0 space-y-3">
          ...
          >
            <p className="mb-3 text-xs text-slate-500">
              ...
              {" "}Full/Sync calls the update-code API to create and sync the dedicated Git Auto project folder, then calls dashboard-state to refresh the view.
            </p>
          ...
          >
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              {workflow?.pull_request?.exists && workflow.pull_request.state?.toLowerCase() === "open" && (
                <Button onClick={() => void openPullRequestDetails()} disabled={pullRequestLoading}>
                  {pullRequestLoading ? "Loading…" : "Open Details"}
                </Button>
              )}
            </div>
            <p className={`mt-3 text-[11px] ${/failed|error/i.test(pullRequestNotice) ? "text-rose-600" : (pullRequestNotice || workflow?.pull_request?.state)?.toLowerCase().includes("open") ? "font-semibold text-emerald-600" : "text-slate-400"}`}>
              {pullRequestNotice || (workflow?.pull_request?.exists
